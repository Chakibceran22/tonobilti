...
type FlexibleTranslationFn = (key: any) => string
...
const CarListingPage: React.FC = () => {
  ...
  const {data: cars, isLoading: loading, error, isError, refetch} = useQuery<CarData[]>({
    queryKey:['cars'],
  ...
  // Query for favorite car IDs
  const { data: favoriteIds, isLoading: favoritesLoading } = useQuery<string[]>({
    queryKey: ['favoriteIds'],
    queryFn: () => userService.getFavoriteCarIds(user!.id),
    enabled: !!user?.id,
    staleTime: 5 * 60 * 1000
  })
  ...
  const addFavoriteMutation = useMutation({
    mutationFn: ({ userId, carId }: { userId: string; carId: string }) => 
      userService.addFavorite(userId, carId),
    onSuccess: () => {
  ...
  const [selectedQuickFilter, setSelectedQuickFilter] = useState<'all' | 'electric' | 'bus'>('all')
  ...
  // Use filtered cars or original cars for display
  const displayCars = filteredCars.length > 0 || selectedQuickFilter !== 'all' ? filteredCars : (cars || [])
  ...
  const handleFilter = (): void => {
    ...
    const filtered = cars.filter((car) => {
      ...
      const makerMatch = filter.maker === "All Makes" ? true : car?.maker?.trim().toLowerCase() === filter.maker.trim().toLowerCase()
      
      // Year filter
      const yearMatch = filter.year === "All Years" ? true : car?.year.toString() === filter.year
      
      // Fuel filter - compare with English values, not translations
      const fuelMatch = filter.fuel === "All Types" ? true : car?.fuel?.trim().toLowerCase() === filter.fuel.trim().toLowerCase()
      
      // Body type filter
      const bodyTypeMatch = filter.bodyType === "All Body Types" ? true : car?.bodyType?.trim().toLowerCase() === filter.bodyType.trim().toLowerCase()

      const result =  makerMatch && yearMatch && fuelMatch && bodyTypeMatch;
      
      if (!result) {
        console.log(`❌ Car filtered out: ${car.title}`, {
           makerMatch, yearMatch, fuelMatch, bodyTypeMatch,
          carData: { maker: car.maker, fuel: car.fuel, year: car.year, bodyType: car.bodyType, price: car.price }
        });
      }
  ...
  // Car card component
 const CarCard: React.FC<CarCardProps> = ({ car,  userInfo, t, isRtl }) => {
    const [isFavorite, setIsFavorite] = useState<boolean>(false)
    
    useEffect(() => {
      setIsFavorite(userInfo?.favoriteCars?.includes(car?.id) || false)
    }, [userInfo, car?.id])
    
    const toggleFavorite = async (e: React.MouseEvent): Promise<void> => {
      // Prevent event from bubbling up to parent (navigation)
      e.stopPropagation();
      ...
      try {
        ...
        if (!isFavorite) {
          await addFavoriteMutation.mutateAsync({ userId: user.id, carId: car.id })
          setIsFavorite(true)
        } else {
          await removeFavoriteMutation.mutateAsync({ userId: user.id, carId: car.id })
          setIsFavorite(false)
        }
      } catch (error) {
        console.log(error)
    ...
    const handleViewDetailsClick = (e: React.MouseEvent): void => {
      e.stopPropagation();
      router.push("/product?id=" + car?.id);
    }
    ...
    return  (
      <div onClick={handleCardClick} className="bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl border border-gray-100 group hover:-translate-y-1">
        <div className="relative">
          <img
            src={car?.images?.[0] || "/placeholder.svg"}
            alt={car?.title}
            className="w-full h-54 sm:h-54 object-cover transition-transform duration-500 group-hover:scale-105"
            loading="lazy"
          />
  
          {car?.available !== undefined && (
            <div
              className={`absolute top-2 left-2 ${car?.available ? "bg-blue-800" : "bg-gray-600"} text-white text-xs font-bold px-2 py-1 rounded-md flex items-center`}
            >
              {car?.available ? (
                <>
                  <span className="w-2 h-2 bg-green-500 rounded-full mx-1.5 "></span>
                  {t('user_available')}
                </>
              ) : (
                <>
                  <X className="w-4 h-4 text-red-500  mx-1.5 a"></X>
                  {t('user_notAvailable')}
                </>
              )}
            </div>
          ...
          >
            {user ? (isFavorite ? <Heart className="h-5 w-5 fill-red-600 border-red-600" /> : <Heart className="h-5 w-5" />) : null}
          </button>
        </div>
        {/* Rest of grid view content */}
        <div className="p-4 sm:p-5">
          <div className="mb-2">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-1 line-clamp-1 group-hover:text-blue-800 transition-colors">
              {car?.title}
            </h3>
          ...
          {/* Performance specs */}
          <div className="grid grid-cols-3 gap-2 mb-3 border-b border-gray-100 pb-3">
            <div className="text-center">
              <p className="text-xs text-gray-500">{t('user_topSpeed')}</p>
              <p className="text-xs sm:text-sm font-semibold text-blue-900">{car?.topSpeed}  km/h</p>
            </div>
            <div className="text-center border-x border-gray-100">
              <p className="text-xs text-gray-500">{t('user_acceleration')}</p>
              <p className="text-xs sm:text-sm font-semibold text-blue-900">{car?.acceleration} sec</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-gray-500">{t('user_horsepower')}</p>
              <p className="text-xs sm:text-sm font-semibold text-blue-900">{car?.horsepower}</p>
            </div>
          </div>
  
          <div className="flex flex-wrap gap-2 sm:gap-3 mb-4">
            <span className="inline-flex items-center text-xs bg-blue-50 px-2 py-1 rounded-full text-blue-800">
              <Calendar className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${isRtl ? "ml-1" : "mr-1"}`} />
              {car?.year}
            </span>
            <span className="inline-flex items-center text-xs bg-blue-50 px-2 py-1 rounded-full text-blue-800">
              <Gauge className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${isRtl ? "ml-1" : "mr-1"}`} />
              {car?.mileage?.toLocaleString() || "0"} {t('user_mi')}
            </span>
            <span className="inline-flex items-center text-xs bg-blue-50 px-2 py-1 rounded-full text-blue-800">
              <Fuel className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${isRtl ? "ml-1" : "mr-1"}`} />
              {car?.fuel}
            </span>
            <span className="inline-flex items-center text-xs bg-blue-50 px-2 py-1 rounded-full text-blue-800">
              <Cog className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${isRtl ? "ml-1" : "mr-1"}`} />
              {car?.transmission}
            </span>
          </div>
          ...
          <div className="flex items-center justify-end">
            <div className="flex space-x-2">
              <button
                className="px-2 sm:px-3 cursor-pointer py-1.5 bg-blue-800 text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-blue-900 transition-colors flex items-center group transform hover:-translate-y-0.5"
                aria-label={`View details for ${car?.title}`}
                onClick={handleViewDetailsClick}
              >
                {t('user_viewDetails')}
                <ChevronRight
                  className={`${isRtl ? "mr-1 rotate-180 group-hover:-translate-x-1" : "ml-1 group-hover:translate-x-1"} h-3 w-3 sm:h-4 sm:w-4 opacity-70  transition-transform duration-300`}
                />
              </button>
            </div>
          </div>
  ...
  const handleFilterChange = (field: keyof FilterState) => (e: React.ChangeEvent<HTMLSelectElement>): void => {
  ...
  return (
    <div
      className={`min-h-screen bg-white overflow-x-hidden overflow-y-auto ${isRtl ? "rtl" : "ltr"}`}
      dir={isRtl ? "rtl" : "ltr"}
    >
      {/* Header */}
      <Header />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 sm:mb-10">
          <div className="mb-6 md:mb-0 animate-fadeInUp">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2">{t('user_discoverPerfectVehicle')}</h2>
            <p className="text-blue-800 max-w-lg">{t('user_browseExclusiveSelection')}</p>
          </div>
          <div className="relative animate-fadeInRight">
            <div className="flex w-full md:w-80">
              <div className="relative w-full">
                <input
                  type="text"
                  placeholder={isError ? t('user_searchDisabled') || 'Search disabled' : t('user_searchCars')}
        ...
        <div
          ref={filterRef}
          id="filter-panel"
          className={`bg-white rounded-xl shadow-md p-6 mb-8 transition-all duration-300 border border-blue-100 ${filterOpen ? "max-h-[1000px] opacity-100 overflow-visible" : "max-h-0 opacity-0 overflow-hidden"
            }`}
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              <Filter className="h-5 w-5 text-blue-800 mx-2" aria-hidden="true" />
              <h3 className="text-lg font-bold text-gray-900">{t('user_refineSearch')}</h3>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 flex items-center" htmlFor="make">
              ...
              >
                ...
                {[...new Set((cars || []).map(car => car.maker))].sort().map(maker => (
                  <option key={maker} value={maker}>{maker}</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 flex items-center" htmlFor="year">
                <span className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-800 mx-2">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 flex items-center" htmlFor="fuel">
                <span className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-800 mx-2">
              ...
              >
                ...
                {[...new Set((cars || []).map(car => car.fuel).filter(Boolean))].sort().map(fuel => (
                  <option key={fuel} value={fuel}>{fuel}</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 flex items-center" htmlFor="bodyType">
                <span className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-800 mx-2">
                  <Car className="h-4 w-4" />
                </span>
                Body Type
              ...
              >
                ...
                {[...new Set((cars || []).map(car => car.bodyType).filter(Boolean))].sort().map(bodyType => (
          ...
          <div className="mt-8 flex justify-center">
            ...
            >
              {t('user_applyFilters')}
              <ArrowRight
                className={`${isRtl ? "mr-2 group-hover:-translate-x-1 rotate-180" : "ml-2 group-hover:translate-x-1"} h-5 w-5 transform transition-transform duration-300`}
              />
            </button>
          </div>
        </div>
  ...
  <div className="flex flex-wrap gap-2 sm:gap-3">
    ...
    >
      <Car className={`h-4 w-4 sm:h-5 sm:w-5 ${isRtl ? 'ml-2' : 'mr-2'}`} />
      <span className="whitespace-nowrap">{t('user_allCars')}</span>
    </button>
    <button
      onClick={isError ? undefined : () => handleQuickFilter('electric')}
      disabled={isError}
      className={`px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-medium rounded-lg transition-all duration-300 flex items-center justify-center shadow-sm border min-w-[120px] sm:min-w-[140px] ${
        isError
          ? 'bg-gray-100 text-gray-400 border-gray-300 cursor-not-allowed'
    ...
    >
      <Fuel className={`h-4 w-4 sm:h-5 sm:w-5 ${isRtl ? 'ml-2' : 'mr-2'}`} />
      ...
      <span className="whitespace-nowrap">{t('user_bus')}</span>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 bg-white p-4 rounded-xl shadow-sm border border-blue-100">
          <div className="flex items-center mb-4 sm:mb-0">
            ...
            <span className="text-gray-700">
              {loading ? (
                ...
                <span className="text-red-600">{t('user_errorLoadingData') || 'Error loading data'}</span>
              ) : (
                <>
                  {t('user_showing')} <span className="font-medium text-blue-800">{currentCars.length}</span> {t('user_of')} <span className="font-medium text-blue-800">{displayCars.length}</span> {t('user_results')}
                </>
              )}
            </span>
          </div>
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center">
              <span className="text-sm text-gray-600 mx-2">{t('user_sortBy')}</span>
              <div className="relative">
                <select
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-20 bg-white rounded-xl shadow-md border border-blue-100">
            ...
            <p className="mt-4 text-lg font-medium text-blue-800">{t('user_loadingPremiumVehicles')}</p>
          </div>
        ) : isError ? (
          <div className="flex flex-col justify-center items-center py-20 bg-white rounded-xl shadow-md border border-red-100">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">{t('user_errorLoadingCars') || 'Failed to Load Vehicles'}</h3>
            <p className="text-gray-600 mb-6 text-center max-w-md">
              {error?.message || t('user_errorLoadingMessage') || 'We encountered an issue while loading the vehicles. Please check your connection and try again.'}
            </p>
          ...
          <div className="flex flex-col justify-center items-center py-20 bg-white rounded-xl shadow-md border border-blue-100">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">{t('user_noVehiclesFound')}</h3>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {currentCars.map((car) => (
                <CarCard key={car?.id} userInfo={userInfo} car={car} t={t} isRtl={isRtl} />
              ))}
            </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-blue-100 p-6">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="text-sm text-gray-600">
                    {t('user_showing')} <span className="font-medium text-blue-800">{indexOfFirstCar + 1}</span> - <span className="font-medium text-blue-800">{Math.min(indexOfLastCar, displayCars.length)}</span> {t('user_of')} <span className="font-medium text-blue-800">{displayCars.length}</span> {t('user_results')}
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <span className="text-gray-600">{t('user_goToPage') || 'Go to page'}</span>
                    <input
                      type="number"
                      min={1}
                      max={totalPages}
